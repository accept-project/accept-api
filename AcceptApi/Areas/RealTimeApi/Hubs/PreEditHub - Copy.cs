...
namespace AcceptApi.Areas.RealTimeApi.Hubs
{
    ...
    public class PreEditHub:Hub
    {
        ...
        private int timeoutThreshould = 10;
        ...
        {           
            ...
            {                               
                ...
                //string text = HttpUtility.UrlDecode(textSegments[0], System.Text.Encoding.UTF8);
                ...
                PreEditWorker(parameters, textSegments, origin, string.Empty);               
            }
            catch (Exception e)
            {
        ...
        private async void PreEditWorker(Dictionary<string, string> parameters, string[] textSegments, string requestOrigin, string initialSessionCodeId)
        {           
            for (int i = 0; i < textSegments.Length; i++)
            {
                try
                {
                    ...
                    if (Context != null && Context.ConnectionId != null && segmentResponse != null && segmentResponse.ResponseStatus == CoreApiResponseStatus.Ok.ToString())
                        Clients.Client(Context.ConnectionId).handleAcceptResponse(Context.ConnectionId, segmentResponse, i);
                    else
                        Clients.Client(Context.ConnectionId).handleAcceptResponseException(Context.ConnectionId, segmentResponse, i);                                      
                }
                    ...
                    while (i < 10)
                    {
                        ...
                        if (segmentStatus != null && segmentStatus.ResponseStatus == CoreApiResponseStatus.Ok)
                        {
                            if (segmentStatus.State == "DONE")
                            {
                                ...
                                return GetResponse(model.AcceptSessionCode, model.GlobalSessionId);
                            }
                            else
        ...
        private CoreApiResponse GetResponse(string session, string globalSessionId)
        {
            ...
            try
            {
                ...
                return new AcceptApiResponseStatus(CoreApiResponseStatus.Failed, session);
                //throw new Exception("Exception Thrown a GetResponse Loop: " + e.Message); 
            }
        }
        ...
        private CoreApiResponse GetResponseStatus(string session)
        {
            try
            {
                ...
                return new AcceptApiResponseStatus(CoreApiResponseStatus.Failed, session);
                //throw new Exception("Exception Thrown a GetResponseStatus Loop: " + e.Message);
            }
        }
        ...
        {
            //CoreApiRequest coreRequest; coreRequest = new CoreApiRequest();
            try
            {
